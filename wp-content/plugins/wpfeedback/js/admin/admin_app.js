var box = '', comments=false, browser='', device_type='', resolution = window.screen.width+' x '+window.screen.height, new_task = [], task_screenshot=[], rightArrowParents = [], current_html_element='', relative_location={}, html_element_location={}, html_element_height=0, html_element_width=0, tasks_on_page = [], onload_wpfb_tasks = [], all_page_tasks_loaded=0, wpf_tasks_loaded=false,all_frontend_tasks_loaded=0,wpf_clean_dom_elem_path='', temp_tasks = [], wpf_tab_permission = {'user':false,'status':false,'priority':false,'screenshot':false,'information':false,'delete_task':'no','auto_screenshot':false},wpf_reconnect_taskid='',wpf_reconnect = false,wpf_reconnect_meta = [],wpf_tag_initialized = [];
/*var comment_count=1;*/
...
function enable_comment(){
    ...
    if(wpf_reconnect == true){
        ...
        jQuery_WPF('body').css('cursor','url("'+wpf_reconnect_icon+'"), crosshair');
        jQuery_WPF('a').css('cursor','url("'+wpf_reconnect_icon+'"), crosshair');
    }
    else{
...
jQuery_WPF(window).on('load', function(){
    if(disable_for_admin == 0){
        ...
        jQuery_WPF( function() {
        jQuery_WPF( ".wpf_launch_buttons" ).draggable({ containment: "#wpwrap" });
    });
    }
...
function generate_task(id){
    ...
    if(wpf_is_valid_url_admin(task_comment) == true){
        temp_task_text = wpf_is_valid_video_url_admin(task_comment);
    }
        ...
        success : function(data){       
            ...
            if(data!=0){
                ...
                if(new_task['task_type']=='general'){
                    ...
                    jQuery_WPF('#wpf_thispage_container').prepend('<li class="current_page_task open" data-taskid="'+id+'"><div class="wpf_task_number">'+id+'</div><div class="wpf_task_sum"><level class="task-author">'+current_user_name+' '+wpf_just_now+' </level><div class="current_page_task_list">'+task_comment+'</div></div><div class="wpf_task_meta"><div class="wpf_task_meta_icon"><i class="gg-chevron-left"></i></div><div class="wpf_task_meta_details">'+wpf_task_status_label+wpf_task_priority_label+wpfb_tags_html+'</div></div></li>');
                }
...
function load_wpfb_tasks_admin(){
    jQuery_WPF.ajax({
        method:"POST",
        url: ajaxurl,
        data: {action:'load_wpfb_tasks_admin',wpf_nonce:wpf_nonce,wpf_current_screen:wpf_current_screen,all_page:0},
        success: function(data){
            ...
            jQuery_WPF.each(k,function (index, value) {
                ...
                jQuery_WPF( function() {
                    jQuery_WPF( "#bubble-"+onload_wpfb_tasks[value].task_comment_id).draggable();
                });
            ...
            jQuery_WPF(document).find("#wpf_frontend_container").on("click","li.current_page_task",function(e) {
                 var task_url = jQuery_WPF(this).attr('data-task_url');
    ...
    if(wpfb_metas.task_type=='general'){
        ...
        for (var key in wpfb_metas.comments) {
            ...
            if(wpfb_metas.comments[key].comment_type==0){
                ...
                wpfb_messages_html += '<li class="'+task_author+'"><level class="task-author">'+wpfb_metas.comments[key].author+'<span>'+wpfb_metas.comments[key].time+'</span></level><div class="meassage_area_main"><a href="'+wpfb_metas.comments[key].message+'" target="_blank"><img src="'+wpfb_metas.comments[key].message+'" class="wpf_task_uploaded_image" /></a></div></li>';
            }
            else{
...
function wpf_task_popover_html(wpf_task_type,comment_count,wpfb_task_id,wpfb_metas,wpfb_users_html,wpfb_task_priority_html,wpfb_task_status_html,wpfb_messages_html) {
    ...
    if(wpf_tab_permission.priority==true){
        tab_nav_html+='<li class="nav-item" title="Status"><a class="nav-link wpf_prio_tab" id="wpfbpriority-tab-'+comment_count+'" data-toggle="tab" href="#wpfbpriority-'+comment_count+'" role="tab" aria-controls="wpfbpriority" aria-selected="false"><i class="gg-danger"></i></a></li>';
    }
    if(wpf_tab_permission.status==true){
        tab_nav_html+='<li class="nav-item" title="Priority"><a class="nav-link wpf_stat_tab" id="wpfbtaskstatus-tab-'+comment_count+'" data-toggle="tab" href="#wpfbtaskstatus-'+comment_count+'" role="tab" aria-controls="wpfbtaskstatus" aria-selected="false"><i class="gg-thermostat"></i></a></li>';
    }
    ...
    if(wpf_tab_permission.information==true){
        tab_nav_html+='<li class="nav-item" title="Additional Information"><a class="nav-link wpf_deta_tab" id="wpfbsysinfo-tab-'+comment_count+'" data-toggle="tab" href="#wpfbsysinfo-'+comment_count+'" role="tab" aria-controls="wpfbsysinfo" aria-selected="false"><i class="gg-globe"></i> </a></li>';
    }
    ...
    if(wpf_current_role == 'advisor') {
        wpf_all_tags_html = '<div class="wpf_icon_title"><i class="gg-tag"></i> Custom Tags</div><div class="wpf_tag_autocomplete"><input type="text" name="wpfeedback_tags" class="wpf_tag" value="" id="wpfeedback_tags_' + wpf_config_task_id + '" data-id="' + wpf_config_task_id + '" onkeydown="wpf_search_tags(this,' + wpf_config_task_id + ')"><button class="wpf_tag_submit_btn" onclick="wpf_add_tag(\'wpfeedback_tags_' + wpf_config_task_id + '\')"><i class="gg-corner-down-left"></i></button></div><div id="all_tag_list_' + wpf_config_task_id + '">' + wpf_tag_html + '</div><div class="wpf_icon_title"> Additional Information</div>';
    }
    ...
    if(wpfb_metas.wpf_task_screenshot != undefined){
        ...
        wpf_screenshot = '<a target="_blank" href="#" class="wpf_screenshot_img_link"><img src="#" id="screenshot_img_'+comment_count+'" style="margin-top:10px; border:1px solid #dee2e6;padding: 5px; width:100%; display: none;" /></a>';
    }
...
function openWPFTab_admin(wpfTab) {
    if(wpfTab == 'wpf_allpages'){
        jQuery_WPF('.wpf_sidebar_content').find('#wpf_thispage').hide(); 
        jQuery_WPF('.wpf_sidebar_content').find('#wpf_frontend').hide(); 
        ...
        if(all_page_tasks_loaded==0){
            ...
            load_all_page_tasks_admin();
        }
    }else if(wpfTab == 'wpf_frontend'){
        jQuery_WPF('.wpf_sidebar_content').find('#wpf_allpages').hide(); 
        jQuery_WPF('.wpf_sidebar_content').find('#wpf_frontend').show(); 
        jQuery_WPF('.wpf_sidebar_content').find('#wpf_thispage').hide(); 
        jQuery_WPF('button.wpf_tab_sidebar').removeClass('wpf_active');
        jQuery_WPF('button.wpf_tab_sidebar.'+wpfTab).addClass('wpf_active');
        ...
        if(all_frontend_tasks_loaded==0){
            all_frontend_tasks_loaded=1;
            load_all_frontend_wpfb_tasks();
        }
    }
    else{
        jQuery_WPF('.wpf_sidebar_content').find('#wpf_allpages').hide(); 
...
function load_all_page_tasks_admin(){
    jQuery_WPF.ajax({
        method:"POST",
        url: ajaxurl,
            ...
            success: function (data) {
                ...
                if(response.status==1){
                    ...
                    if(response.type==1){
                        ...
                        var comment_html = '<li class="wpf_author"><level class="task-author">'+current_user_name+'<span>'+wpf_just_now+'</span></level><div class="meassage_area_main"><a href="'+response.message+'" download="'+wpf_download_file+'"><i class="gg-software-download"></i> '+ wpf_download_file+'</a></div></li>';
                    }
...
function wpf_load_general_task(id) {
    jQuery_WPF.ajax({
        url:ajaxurl,
        method:'POST',
        data:{action:'load_wpfb_tasks_admin',wpf_nonce:wpf_nonce,task_id:id},
        beforeSend: function(){
            jQuery_WPF('.wpf_sidebar_loader').show();
        },
        success: function (data) {
...
function load_all_frontend_wpfb_tasks(){
    jQuery_WPF.ajax({
        method:"POST",
        url: ajaxurl,
        ...
        success: function(data){
            ...
            jQuery_WPF("#wpf_frontend_container").html(wpfb_all_page_task_list_html);
        }
    });
}
...
function wpf_is_valid_url_admin(string) {
...
function wpf_is_valid_video_url_admin(string) {
